function [u, B, feas, comp_time] = ctrlCpCbfQp(obj, x, u_ref, cp_quantile)
    %% Implementation of CP-CBF-QP
    % Inputs:   x: state
    ...
    cp_bound = cp_quantile * norm(obj.dcbf(x), 2);
        
    %% Constraints : A * u <= b
    % CBF constraint.
    A = [-LgB];
    b = [LfB - cp_bound + obj.params.cbf.rate * B];
