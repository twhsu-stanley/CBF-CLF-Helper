function initSys(obj, symbolic_x, symbolic_f, symbolic_g, symbolic_cbf, symbolic_clf)
    ...
    if ~isempty(symbolic_cbf)
        ...
        obj.dcbf = matlabFunction(dcbf, 'vars', {x});
        lf_cbf_ = dcbf * f_;
        lg_cbf_ = dcbf * g_;        
        ...
        ddcbf = simplify(hessian(symbolic_cbf, symbolic_x));
        obj.ddcbf = matlabFunction(ddcbf, 'vars', {x});
    end

    % Obtaining Lie derivatives of CLF.    
    if ~isempty(symbolic_clf)
        dclf = simplify(jacobian(symbolic_clf, symbolic_x));
        obj.dclf = matlabFunction(dclf, 'vars', {x});
        lf_clf_ = dclf * f_;
        lg_clf_ = dclf * g_;
        ...
        ddclf = simplify(hessian(symbolic_clf, symbolic_x));
        obj.ddclf = matlabFunction(ddclf, 'vars', {x});
    end
end